...
Buff_DetectorTRT::Buff_DetectorTRT(const std::string & config_path) : detector_(config_path)
{
...
std::optional<PowerRune> Buff_DetectorTRT::detect(cv::Mat & bgr_img)
{
  last_objects_.clear();
...
cv::Point2f Buff_DetectorTRT::fanblade_center(const RuneObject & obj) { return average_fanblade_center(obj); }

float Buff_DetectorTRT::radius_to_fanblade_center(const RuneObject & obj)
{
  return cv::norm(obj.pts.r_center - fanblade_center(obj));
}

float Buff_DetectorTRT::center_angle(const RuneObject & obj)
{
  const auto delta = fanblade_center(obj) - obj.pts.r_center;
  return std::atan2(delta.y, delta.x);
}

std::optional<PowerRune> Buff_DetectorTRT::to_power_rune(const RuneObject & obj)
{
...
void Buff_DetectorTRT::filter_by_radius_ratio(std::vector<RuneObject> & objects) const
{
...
std::optional<RuneObject> Buff_DetectorTRT::select_locked_candidate(
